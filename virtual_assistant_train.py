...
def process_models(path,task):
    ...
    targz_unzip_single_file(path+data_set_zip, task+train_set_file, task)
    targz_unzip_single_file(path+data_set_zip, task+test_set_file, task)


logging.basicConfig(level=logging.DEBUG)
path="./neural/"
tasks_paths = ["tasks_1-20_v1-2/en/"]
...
train_set_post_file =tasks_paths[0] + train_set_file
test_set_post_file = tasks_paths[0] + test_set_file

# process_models(path,path+tasks_paths[0])
...
def contextualize(category, val=False):
    ...
    if val:
        ...
        with open(category, "r", encoding='utf-8-sig') as train:
            for line in train:
...
def session_manage(location, rewrite=False, iter=50000, batch_size=batch_size):
